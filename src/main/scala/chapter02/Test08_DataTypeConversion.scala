...
object Test08_DataTypeConversion {
  def main(args: Array[String]): Unit = {
    ...
    // 1. 自动类型转换
    //    (1) 自动提升原则: 有多重类型的数据混合运算时，系统首先自动将所有数据转化成精度大的那种数据类型，然后再进行计算。
    val a1: Byte = 10
    val b1: Long = 2353L
    val result1: Long = a1 + b1
    val result11: Int = (a1 + b1.toInt) // 强转

    //    (2) 把精度大的数据类型赋值给精度小的数值类型时，就会报错，反之就会进行自动类型转化。
    val a2: Byte = 10
    val b2: Int = a2
    ...
    //    (4) byte, short, char他们之间三者可以计算，在计算时首先转换为int类型。
    val a4: Byte = 12
    val b4: Short = 25
    val c4: Char = 'c'
